...
def create_sh_clients(create_model_fn, nclients, client_ids, seed=None):
    ...
    def create_client(client_id: str):
        ...
        return fl.client.Client(client_data, create_model_fn)
...
def experiment(config, strategy_name, middle_server_class=fl.middle_server.MiddleServer):
    ...
    if config['dataset'] == "fmnist":
        ...
        data_collector_counts, client_ids = fl.load_data.solar_home_customer_regions()
    for i in (pbar := trange(config['repeat'])):
        ...
        if config['dataset'] == "fmnist":
            ...
            clients = create_sh_clients(
                fl.common.create_solar_home_model,
                fl.common.count_clients(network_arch),
                client_ids,
                seed=seed
            )
