...
def create_sh_clients(create_model_fn, nclients, client_ids, seed=None):
    get_customer_data = load_data.solar_home()
    ...
    def create_client(client_id: str):
        ...
        return client.Client(client_data, create_model_fn)
...
def create_clients(data, create_model_fn, network_arch, seed=None):
    ...
    def create_client(client_id: str):
        return client.Client(data.select({"train": next(idx), "test": next(test_idx)}), create_model_fn)
    return create_client
...
def experiment(config, strategy_class, middle_server_class=flagon.MiddleServer):
    ...
    if config['dataset'] == "fmnist":
        ...
        data_collector_counts, client_ids = load_data.solar_home_customer_regions()
    for i in (pbar := trange(config['repeat'])):
        seed = round(np.pi**i + np.exp(i)) % 2**32
        if config['dataset'] == "fmnist":
            server = flagon.Server(common.create_fmnist_model().get_parameters(), config)
            network_arch = {
                "clients": [
                    {"clients": 3, "strategy": strategy_class(), "middle_server_class": middle_server_class} for _ in range(5)
                ]
            }
            clients = create_clients(data, common.create_fmnist_model, network_arch, seed=seed)
        else:
            server = flagon.Server(common.create_solar_home_model().get_parameters(), config)
            network_arch = {
            ...
            clients = create_sh_clients(
                common.create_solar_home_model, flagon.common.count_clients(network_arch), client_ids, seed=seed
            )
...
if __name__ == "__main__":
    ...
    results = experiment(
        experiment_config,
        strategy.KickbackMomentum if experiment_config.get("mu1") else flagon.strategy.FedAVG,
        middle_server.IntermediateFineTuner if experiment_config.get("num_finetune_episodes") else flagon.MiddleServer
    )
